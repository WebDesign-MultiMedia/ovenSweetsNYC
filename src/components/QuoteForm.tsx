...
export default function QuoteForm({ showHeading = true }: QuoteFormProps) {
  ...
  const itemOptions = [
    { value: 'Custom Cake', label: t.quote.itemOptions.customCake },
    { value: 'Flan', label: t.quote.itemOptions.flan },
    { value: 'Gelatina', label: t.quote.itemOptions.gelatina },
    { value: 'Other', label: t.quote.itemOptions.other },
  ]
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!HEROTOFU_ENDPOINT) {
      setStatus('error')
      setErrorMessage(t.quote.errorNotConfigured)
      return
    }
    ...
    try {
      ...
      setStatus('error')
      setErrorMessage(t.quote.errorGeneric)
    }
  }
    ...
    return (
      <section id="quote" className="px-4 py-12 md:px-8 md:py-20">
        <div className="mx-auto max-w-2xl rounded-3xl border border-rose/30 bg-white p-8 text-center shadow-sm">
          <h2 className="font-heading text-2xl font-semibold text-chocolate">{t.quote.thankYouTitle}</h2>
      ...
      <div className="mx-auto max-w-2xl">
        {showHeading && (
          <div className="mb-8 text-center">
            <h2 className="font-heading text-3xl font-semibold text-chocolate md:text-4xl">{t.quote.heading}</h2>
            <p className="mt-2 text-chocolate/70">{t.quote.subheading}</p>
          </div>
        )}
        ...
        >
          ...
          <Field label={t.quote.flavorSelection} htmlFor="flavor_selection">
            <input
              id="flavor_selection"
              name="flavor_selection"
              type="text"
              placeholder={t.quote.flavorPlaceholder}
              className={inputClass}
            />
          </Field>

          <Field label={t.quote.fillingSelection} htmlFor="filling_selection">
            <input
              id="filling_selection"
              name="filling_selection"
              type="text"
              placeholder={t.quote.fillingPlaceholder}
              className={inputClass}
            />
          </Field>

          <Field label={t.quote.inspirationPhoto} htmlFor="inspiration_photo">
            <label
            ...
            >
              ...
              {fileName || t.quote.uploadPlaceholder}
            </label>
            <input
          ...
          <Field label={t.quote.additionalDetails} htmlFor="additional_details">
            <textarea
          ...
          >
            {status === 'submitting' ? t.quote.sending : t.quote.send}
          </button>
        </form>
      </div>
    </section>
